...
export async function createImoCrewListPdf(draft: ImoCrewListDraft): Promise<Blob> {
  ...
  // Keep the desktop table's column proportions inside portrait A4 margins.
  ...
  const setBodyFont = (size = FONT_SIZE) => {
  ...
  if (draft.crew.length === 0) {
    ...
    draft.crew.forEach((crew, index) => {
      ...
      while (offset < lineCount) {
        ...
        const height = Math.max(6.5, count * LINE_HEIGHT + CELL_PADDING * 2);
