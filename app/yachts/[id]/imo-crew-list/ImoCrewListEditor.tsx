...
export default function ImoCrewListEditor({ initialDraft, language }: { initialDraft: ImoCrewListDraft; language: string }) {
  ...
  function addCrew() {
    ...
    requestAnimationFrame(() => document.getElementById(`crew-${row.id}-fullName`)?.focus());
  }
  ...
  return (
    <main className={`bd-app-page ${styles.page}`} data-i18n-ignore>
      <div className={styles.container}>
        ...
        <div className={styles.documentViewport} role="region" aria-label={copy("Editable crew list document", "Doldurulabilir mürettebat listesi belgesi")} tabIndex={0}>
          <form ref={formRef} className={styles.paper} lang="en" onSubmit={(event) => event.preventDefault()} onInputCapture={() => setInputRevision((current) => current + 1)} aria-label="IMO Crew List">
